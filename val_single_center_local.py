...
# python val_single_center_local.py --name XY-local-UNeXt-all299-0804 --input_folder /223010087/SimonWorkspace/paper2/segmentation/UNeXt-pytorch/models/XY-local-UNeXt-First114-0804-cuda1/test-img/ --mask_folder models//XY-local-UNeXt-First114-0804-cuda1//test-img-mask//
...
def main():
    ...
    for c in range(config['num_classes']):
        os.makedirs(os.path.join('outputs', config['name'], 'test-img-center', str(c)), exist_ok=True)
                ...
                for c in range(config['num_classes']):
                    ...
                    for contour in contours:
                        ...
                        if M["m00"] != 0:
                            cX = int(M["m10"] / M["m00"])
                            cY = int(M["m01"] / M["m00"])
                            cv2.circle(output_resized, (cX, cY), 5, (0, 0, 255), -1)  # Draw red point

                    save_path = os.path.join('outputs', config['name'], 'test-img-center', str(c), meta['img_id'][i] + '.jpg')
